...
def update_file_handle(symbol, book_timestamp):
    ...
    for fn in ["snap", "trade", "tick", "liquidations"]:
        pass
    ...
    filename = cur_time.strftime("%Y%m%d") + f"_{section_id:02d}.csv"  # with compression
    ...
    async with QUEUE_LOCK[symbol]:
        ...
        while len(TRADES_QUEUE[symbol]) > 0:
            ...
            if top_queue_ts_diff > _TRADE_TIMEOUT:
                ...
                trades_for_cur_book.append(TRADES_QUEUE[symbol].popleft())
        ...
        while len(LIQUIDATIONS_QUEUE[symbol]) > 0:
            ...
            if top_queue_ts_diff >= 0:
                liquidations_for_cur_book.append(LIQUIDATIONS_QUEUE[symbol].popleft())
        ...
        while len(TICKS_QUEUE[symbol]) > 0:
            ...
            if top_queue_ts_diff >= 0:
                ticks_for_cur_book.append(TICKS_QUEUE[symbol].popleft())
        book_dict["ticks"] = ticks_for_cur_book

    if time.time() - st_time > 5 and len(trades_for_cur_book) > 0:
        # pprint(book_dict)
        pprint(trades_for_cur_book)
...
async def process_trades(trades, receipt_timestamp):
    ...
    async with QUEUE_LOCK[symbol]:
        TRADES_QUEUE[symbol].append((trades.timestamp, receipt_timestamp, raw_resp))
...
async def process_liquidations(liquidations, receipt_timestamp):
    ...
    async with QUEUE_LOCK[symbol]:
        LIQUIDATIONS_QUEUE[symbol].append((liquidations.timestamp, receipt_timestamp, raw_resp))
...
async def process_ticks(ticks, receipt_timestamp):
    ...
    symbol = ticks.symbol
    async with QUEUE_LOCK[symbol]:
        TICKS_QUEUE[symbol].append((ticks.timestamp, receipt_timestamp, raw_resp))
